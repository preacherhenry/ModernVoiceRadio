import Image from "next/image";
import Link from "next/link";
import { Pencil } from "lucide-react";
import AdminPageHeader from "@/components/admin/AdminPageHeader";
import DeleteButton from "@/components/admin/DeleteButton";
import { prisma } from "@/lib/prisma";
import { formatDate } from "@/lib/format";
import { isLocalUpload } from "@/lib/image";
import { deleteEvent } from "./actions";

export default async function AdminEventsPage() {
  const events = await prisma.event.findMany({ orderBy: { date: "asc" } });

  return (
    <div>
      <AdminPageHeader
        eyebrow="Save The Date"
        title="Events"
        newHref="/admin/events/new"
        newLabel="New Event"
      />

      <div className="mt-8 flex flex-col divide-y divide-line border border-line">
        {events.length === 0 && <p className="p-6 text-sm text-grey-500">No events yet.</p>}
        {events.map((e) => (
          <div key={e.id} className="flex items-center gap-4 bg-ink-2 p-4">
            <div className="relative size-14 shrink-0 overflow-hidden">
              <Image src={e.image} alt={e.title} fill unoptimized={isLocalUpload(e.image)} className="object-cover" />
            </div>
            <div className="min-w-0 flex-1">
              <p className="truncate text-sm font-semibold text-white">{e.title}</p>
              <p className="truncate text-xs text-grey-500">
                {formatDate(e.date.toISOString())} · {e.location}
              </p>
            </div>
            <Link
              href={`/admin/events/${e.id}/edit`}
              className="flex items-center gap-1.5 border border-line-strong px-3 py-1.5 font-condensed text-xs font-semibold uppercase tracking-[0.08em] text-grey-300 transition-colors hover:border-gold hover:text-gold"
            >
              <Pencil className="size-3.5" />
              Edit
            </Link>
            <DeleteButton action={deleteEvent.bind(null, e.id)} />
          </div>
        ))}
      </div>
    </div>
  );
}
